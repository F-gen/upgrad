        ...
        >
          <template v-slot:title="nodeData">
            ...
            <div style="float: right">
              <span
                v-if="nodeData.dataRef.name != 'Root'"
...
const addMenu = async (item) => {
  // console.log(item,'addmenu');// 直接打印的值 为reset 后的值
  let items = JSON.parse(JSON.stringify(item));
  // console.log(item,items,'addmenu--josn'); // 拷贝传来的值 有值

  nameKey.value = [];
  getNameKey(treeData.value);
  let isAlreadyinMenu = nameKey.value.some(
    (value) => value.name == items.name || value.tag == items.tag
  );
  if (isAlreadyinMenu) {
    message.warning("菜单名称或标签重复，请重新添加");
    return;
  } else {
    addMenuchildren(treeData.value, items, where.value);
...
const editMenu = async (item) => {
  nameKey.value = [];
  getNameKey(treeData.value);
  nameKey.value = nameKey.value.filter((item) => {
    return item.key !== editTargetKey.value;
  });
  let isAlreadyinMenu = nameKey.value.some(
    (value) => value.name == item.name || value.tag == item.tag
  );
  if (isAlreadyinMenu) {
